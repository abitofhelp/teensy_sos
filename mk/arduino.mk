...
# output/input dirs are ABSOLUTE on purpose: a relative input dir makes the
# Teensy loader fail to read the compiled sketch on upload.
#
# Namespaced targets (arduino-build/-upload/-clean) are always available. The
# generic build/upload/monitor verbs bind here only when BUILDER=arduino.
# =============================================================================

ARDUINO_CLI      := $(shell command -v arduino-cli 2>/dev/null)
ARDUINO_CLI_NAME := arduino-cli
ARDUINO_CLI_HINT := brew install arduino-cli (or https://arduino.github.io/arduino-cli/); only for 'make arduino-*' / BUILDER=arduino.
...
ARDUINO_HEX      := $(ARDUINO_BUILD)/$(notdir $(ARDUINO_SKETCH)).ino.hex
...
.PHONY: arduino-check arduino-build arduino-upload arduino-clean arduino-monitor
...
arduino-build: arduino-check ## Compile the sketch with GNU C++20 against the canonical library (absolute output dir)
...
arduino-monitor: ## Open the Arduino CLI serial monitor (dynamic port; override ARDUINO_PORT=...)
	@port="$(ARDUINO_PORT)"; \
...
# --- aggregate contributions (always, whether or not arduino is the selected builder)
clean::
	rm -rf "$(ARDUINO_BUILD)"
